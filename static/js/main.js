...
var huawei = {
  init: function () {
    ...
    $(".lazy").lazyload({
      ...
      effect: "fadeIn",
      failure_limit: 20,
      placeholder: "/Assets/CBG/img/lazy.png"
    });
  ...
  mastheadMenu: function () {
    ...
    }, ">li>a").off("vclick", ">li>a").on("vclick", ">li>a", function (al) {
    ...
    function aa(ai) {
      $(".hw1_menu_section[data-section='" + ai + "']").find(".hw1_box_wrap").outerHeight("");
...
function GetUserInfo() {
  var IsLogin = GetSpaceCookieValue("hwsso_uniportal");
  var IsBackLogin = GetSpaceCookieValue("login_uid");//后台登录
...
/*
 ...
 参数：name，cookie名字
 */
function GetSpaceCookieValue(name) {
...
huawei.index_wookmark2 = function () {
  var $container = $('.wookmark2');
  if (!$container.length)
    return;
  $('#container').imagesLoaded(function () {
    // images have loaded
  ...
  $container.infinitescroll({
      ...
      loading: {
        finishedMsg: $("#ComfirmNoMore").val(),//'No more pages to load.',
        msgText: $("#LoadingNext").val(),
        img: 'http://{{.Web_url}}:8080/search/pages/search/img/ajax.gif'
      },
      path: function (index) {
        return $("#page-nav a").attr("href") + "&pageIndex=" + index;
      }
    },
 ...
 */
function hwaTrackEventClick(pC, pEName, pG, pF, pNowss) {
  /*以上是事例Demo
   ha("set", {
  ...
  try {
    ha("set", {
      "opr_wf_n": pEName,
      "opr_wf_d": (new Date()).getTime() - pNowss,
...
huawei.combobox_readonly = function () {
  ...
  setTimeout(function () {
    $(".solar-down-page-cell .ui-autocomplete").hide();
  }, 150);
...
//首页默认不加载验证码，在点击icon_news事件时才加载 xwx282787
function clickNewsVerify() {
  $("input[name='VerificationCode']").val("");
  $("#VerifyCode").click();
}
        ...
        .validate({
          ...
          rules: {
            Email: {
              ...
              remote: {
                url: $("#AccountModifyOrRegServiceUrl").val() + "?type=5",
...
function SubmitRegisterLogin() {
  try {
    ...
    ////xwx282787语种mm, en-mm注册完邮件中改成默认成功地址
    //if ($("#languages").val() == 'en-mm')
    //{
    //    $("#appurl").val("http://www-beta.huawei.com/minisite/better-connected-myanmar/en/register-activity/success.html");
    //}
    //if ($("#languages").val() == 'mm') {
    //    $("#appurl").val("http://www.huawei.com/minisite/better-connected-myanmar/mm/register-activity/success.html");
    //}
...
function Usearch(obj) {
  ...
  if ($("#psirtId").val() != undefined && $("#psirtId").val() == "psirtId") {
    if (language == "cn") {
      var url = "http://{{.Web_url}}:8080/search/#permissionSwitch=false&resType=psirt&searchType=ST-BULLETINS&searchString=" + keyword;
...
function redirectUrl(keyword, language) {
  if (language == "cn") {
    var url = "http://{{.Web_url}}:8080/search/?lang=zh#currentPage=1&resLang=zh&searchType=www_all&searchString=" + keyword;
    $("#linkUrl").attr("href", url);
    $("#forward").click();
  } else if (language == "en") {
