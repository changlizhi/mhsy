function Usearch(obj) {
  ...
  if ($("#psirtId").val() != undefined && $("#psirtId").val() == "psirtId") {
    if (language == "cn") {
      //window.open("http://{{.Web_url}}:8080search/#permissionSwitch=false&resType=psirt&searchType=ST-BULLETINS&searchString=" + keyword);
...
function Usearchs(obj) {
  ...
  if ($("#psirtId").val() != undefined && $("#psirtId").val() == "psirtId") {
    if (language == "cn") {
      //window.open("http://{{.Web_url}}:8080/search/#permissionSwitch=false&resType=psirt&searchType=ST-BULLETINS&searchString=" + keyword);
      var url = "http://{{.Web_url}}:8080/search/#permissionSwitch=false&resType=psirt&searchType=ST-BULLETINS&searchString=" + keyword;
      $("#linkUrl").attr("href", url);
      $("#forward").click();
    } else if (language == "en") {
...
function redirectUrl(keyword, language) {
  if (language == "cn") {
    //window.open("http://{{.Web_url}}:8080/search/?lang=zh#currentPage=1&resLang=zh&searchType=www_all&searchString=" + keyword);
